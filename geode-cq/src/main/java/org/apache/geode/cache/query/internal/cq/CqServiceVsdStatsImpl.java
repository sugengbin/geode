...
public class CqServiceVsdStatsImpl implements CqServiceVsdStats, GFSStatsImplementor {
   ...
   */
  private int _numCqsCreatedId;

  /**
   * Id of the active CQs statistic
   */
  private int _numCqsActiveId;

  /**
   * Id of the stopped CQs statistic
   */
  private int _numCqsStoppedId;

  /**
   * Id of the closed CQs statistic
   */
  private int _numCqsClosedId;

  /**
   * Id of the CQs on client statistic
   */
  private int _numCqsOnClientId;

  /**
   * Id of the Clients with Cqs statistic
   */
  private int _numClientsWithCqsId;

  /**
   * Id for the CQ query execution time.
   */
  private int _cqQueryExecutionTimeId;

  /**
   * Id for the CQ query execution in progress
   */
  private int _cqQueryExecutionInProgressId;

  /**
   * Id for completed CQ query executions
   */
  private int _cqQueryExecutionsCompletedId;

  /**
   * Id for unique CQs, difference in CQ queries
   */
  private int _numUniqueCqQuery;
    ...
    _type = factory.createType(statName, statName,
        new StatisticDescriptor[]{
            ...
            factory.createLongCounter(CQS_ACTIVE, "Number of CQS actively executing.",
                "operations"),
            factory.createLongCounter(CQS_STOPPED, "Number of CQs stopped.", "operations"),
            factory.createLongCounter(CQS_CLOSED, "Number of CQs closed.", "operations"),
            factory.createLongCounter(CQS_ON_CLIENT, "Number of CQs on the client.", "operations"),
            factory.createLongCounter(CLIENTS_WITH_CQS, "Number of Clients with CQs.",
                "operations"),
            factory.createLongCounter(CQ_QUERY_EXECUTION_TIME, "Time taken for CQ Query Execution.",
                "nanoseconds"),
            factory.createLongCounter(CQ_QUERY_EXECUTIONS_COMPLETED,
                "Number of CQ Query Executions.",
                "operations"),
            factory.createIntGauge(CQ_QUERY_EXECUTION_IN_PROGRESS,
                "CQ Query Execution In Progress.",
                "operations"),
            factory.createIntGauge(UNIQUE_CQ_QUERY, "Number of Unique CQ Querys.", "Queries"),
